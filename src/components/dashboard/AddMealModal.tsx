    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="card w-full max-w-md mx-4 shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
          {filtered.length === 0 && <p className="text-center text-gray-400 text-sm py-8">No recipes found</p>}
          {filtered.map((recipe) => (
            <button key={recipe.id} onClick={() => setSelected(recipe)}
              className={`w-full text-left px-3 py-2.5 rounded-xl transition-all flex items-center gap-3
                ${selected?.id === recipe.id ? 'bg-brand-50 border border-brand-200' : 'hover:bg-gray-50 border border-transparent'}`}>
              <span className="text-xl">{recipe.emoji}</span>
              <div className="min-w-0">
                <p className="font-semibold text-sm text-gray-900 truncate">{recipe.name}</p>
                <p className="text-xs text-gray-400">{recipe.macrosPerServing.calories} kcal · {recipe.macrosPerServing.protein}g protein</p>
              </div>
              {selected?.id === recipe.id && (
                <div className="ml-auto w-4 h-4 rounded-full bg-brand-500 flex items-center justify-center shrink-0">
          ...
          <div className="px-5 py-4 border-t border-gray-100 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button className="btn-secondary btn-icon w-8 h-8 text-lg" onClick={() => setServings((s) => Math.max(1, s - 1))}>−</button>
                <span className="w-6 text-center font-bold font-mono text-sm">{servings}</span>
                <button className="btn-secondary btn-icon w-8 h-8 text-lg" onClick={() => setServings((s) => s + 1)}>+</button>
              </div>
            </div>
